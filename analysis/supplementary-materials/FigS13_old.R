...
k2o_sio2 <- ggplot(data=df, mapping=aes(x=x, y=y)) + geom_blank() +
  ...
  annotate("text", label="Basalt", x=47.5, y=4.5, size=1, colour="black")+
  annotate("text", label="Basaltic\n andesite", x=54, y=4.5, size=1, colour="black")+
  annotate("text", label="Andesite", x=59.5, y=4.5, size=1, colour="black")+
  annotate("text", label="Dacite", x=66.5, y=4.5, size=1, colour="black")+
  annotate("text", label="Rhyolite", x=74, y=4.5, size=1, colour="black")+
...
shapes <- c("[ANU9006] Vanua Lava (Vanuatu)"=0,
            "[ANU9009] Vanua Lava (Vanuatu)"=1,
            "[ANU9008] Vanua Lava (Vanuatu)"=2,
            "[ANU9003] Vanua Lava (Vanuatu)"=5,"E-11-03"=22)
cols <- c("[ANU9006] Vanua Lava (Vanuatu)"="#7AD04F",
          "[ANU9009] Vanua Lava (Vanuatu)"="#7AD04F",
          "[ANU9008] Vanua Lava (Vanuatu)"="#7AD04F",
          "[ANU9003] Vanua Lava (Vanuatu)"="#7AD04F","E-11-03"="red")
contour <- c("[ANU9006] Vanua Lava (Vanuatu)"="#7AD04F",
             "[ANU9009] Vanua Lava (Vanuatu)"="#7AD04F",
             "[ANU9008] Vanua Lava (Vanuatu)"="#7AD04F",
             "[ANU9003] Vanua Lava (Vanuatu)"="#7AD04F","E-11-03"="black")

E_11_03 <- k2o_sio2 +
  geom_point(data=d,
             aes(x=SiO2, y=K2O, shape=factor(Location), color=factor(Location),
                 fill=factor(Location), group=Sample), size=1.5, stroke=.25) +
  geom_point(data=s,aes(x=SiO2, y=K2O, shape=factor(Location),
                        color=factor(Location), fill=factor(Location),
                        group=Sample), size=1.5, stroke=.5) +
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size=0.5),
        ...
        axis.text = element_text(size = 7), axis.title.x = element_blank(),
...
OR id='1141527' OR id='70517'") %>%
  ...
  dplyr::mutate(Island = case_when(grepl("MANADO TUA", LOCATION) ~ "Manadotua",
...
d_s <- joined_data %>% filter(Sample %in% c("E-11-11")) %>%
  dplyr::mutate(Location = case_when(grepl("E-11-11", Sample) ~ "Vanuatu")) %>%
  dplyr::mutate(Island = case_when(grepl("E-11-11", Sample) ~ "Emae")) %>%
  dplyr::select(Sample,Location,lat,long,SiO2,TiO2,Al2O3,MnO,MgO,CaO,Na2O,K2O,
                Li,Sc,Ti,V,Cr,Co,Ni,Cu,Zn,As,Rb,Sr,Y,Zr,Nb,Cd,Cs,Ba,La,Ce,Pr,Nd,
                Sm,Eu,Gd,Tb,Dy,Ho,Er,Tm,Yb,Lu,Hf,Ta,Pb,Th,U,K,
                Sr87_Sr86,Nd143_Nd144,Pb206_Pb204,Pb207_Pb204,Pb208_Pb204)
...
E_11_0607 <- k2o_sio2 +
  geom_point(data=d,
             aes(x=SiO2, y=K2O, shape=factor(Location), color=factor(Location),
                 group=Sample), size=1.5, stroke=.25) +
  geom_point(data=s,aes(x=SiO2, y=K2O, shape=factor(Location),
                        color=factor(Location), group=Sample), size=1.5, stroke=.5) +
  scale_shape_manual(values = shapes) + scale_color_manual(values = cols) +
  scale_y_continuous(position = "right") +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=0.5),
        panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
...
pdf(here("analysis","supplementary-materials","FigS13","FigS13-c(bis).pdf"), width=5, height=2)
K_12_28_spider_minmax
dev.off()
...
pdf(here("analysis","supplementary-materials","FigS13","FigS13-d(bis).pdf"), width=5, height=2)
K_12_29_spider_minmax
dev.off()



#### classification E_11_03 ####
...
shapes <- c("Ulawun"=21,"Lolobau"=22,"Sulu"=23,"Wulai"=24,"Manam"=25,"K-12-28"=3)
cols <- c("Ulawun"="#25A782","Lolobau"="#25A782","Sulu"="#25A782",
          "Wulai"="#25A782","Manam"="#25A782","K-12-28"="red")
contour <- c("Ulawun"="black","Lolobau"="black","Sulu"="black",
             "Wulai"="black","Manam"="black","K-12-28"="red")
...
shapes <- c("Cebu"=21,"Buhia"=22,"Ureparapara"=23,"Vanua Lava"=24,
            "Kibobo"=25,"K-12-29"=4)
cols <- c("Cebu"="#440154","Buhia"="#345E8C","Ureparapara"="#7AD04F",
          "Vanua Lava"="#7AD04F","Kibobo"="#BADD26","K-12-29"="red")
contour <- c("Cebu"="black","Buhia"="black","Ureparapara"="black",
             "Vanua Lava"="black","Kibobo"="black","K-12-29"="red")

K_12_29 <- k2o_sio2 +
  geom_point(data=d,
             aes(x=SiO2, y=K2O, shape=factor(Location), color=factor(Location),
                 fill=factor(Location), group=Sample), size=1, stroke=.25) +
  geom_point(data=s,aes(x=SiO2, y=K2O, shape=factor(Location),
                        color=factor(Location), fill=factor(Location),
                        group=Sample), size=1, stroke=.5) +
  scale_shape_manual(values = shapes) +
  scale_fill_manual(values = cols) + scale_color_manual(values = contour) +
  scale_y_continuous(position = "right") +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=0.5),
